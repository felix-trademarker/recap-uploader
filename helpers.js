...
exports.getFiles = function(path, extension) {
  let files = fs.readdirSync( path );
  return files.filter( file => file.match(new RegExp(`.*\.(${extension})`, 'ig')));
}

exports.saveToS3 = async function(imageServerUrl, filename) {
  ...
  const imagesRes = imageServerUrl
  const blob = fs.readFileSync(imagesRes)
